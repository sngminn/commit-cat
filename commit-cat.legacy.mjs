...
function wrapText(text, maxWidth) {
  if (!text) return "";
  const limit = maxWidth || Math.min(80, process.stdout.columns - 12);
...
async function main() {
  ...
  try {
    ...
    p.note(chalk.red(wrapText(e.message)), "AI Error");
    process.exit(1);
  }
  ...
  if (review?.critical?.length > 0) {
    ...
    review.critical.forEach((c) => {
      console.log(
        chalk.red(` [${c.filePath}:${c.lineNumber || "?"}] ${c.message}`)
      );
    });
  }
    ...
    const selected = handleCancel(
      await p.multiselect({
        ...
        options: review.suggestions.map((s, idx) => {
          ...
          const prefix = `[${s.filePath}:${s.lineNumber || "?"}]`;
          const context = s.contextLine
            ? `\n${chalk.dim(s.contextLine.trim())}`
            : "";
          // Extra padding for multiselect indentation (~4-5 chars) + border
          const safeWidth = Math.max(40, process.stdout.columns - 20);
          const body = wrapText(s.message, safeWidth);
